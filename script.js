document.addEventListener("DOMContentLoaded", () => {
  ...
  navLinks.forEach((link) => {
    link.addEventListener("click", (e) => {
      // Only prevent default for anchor links (links starting with #)
      if (link.getAttribute("href").startsWith("#")) {
        e.preventDefault();
        const targetId = link.getAttribute("href");
        const targetElement = document.querySelector(targetId);
  ...
  function typeWriter() {
    if (currentParagraph < aboutParagraphs.length) {
      ...
      if (currentChar < text.length) {
        paragraph.innerHTML =
          text.slice(0, currentChar + 1) + '<span class="cursor">|</span>';
        currentChar++;
      ...
      aboutParagraphs[aboutParagraphs.length - 1].innerHTML =
        aboutParagraphs[aboutParagraphs.length - 1].textContent;
    }
  }
